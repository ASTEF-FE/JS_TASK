...
function f11() {
    let num = Number(i11.value);
    console.log(num);
    let out11 = '';
    for (i = 0; i < d11.length; i++) {
        if (d11.indexOf(num) == -1) {
            out11 = 1;
        } else {
            out11 = d11.indexOf(num);
        }
    }
...
function f12() {
    ...
    let out12 = '';
    let num = Number(i12.value);
    for (let i = 0; i < d12.length; i++) {

        if (d12[i] === num) {
            out12 = ` ${i} `;
            console.log(i);
            break; // полностью закончит работу цикла (начнет работать  строка *) когда он выведит индекс совпавшего элемента 
        } else {
...
function f15() {
    let num = Number(i15.value);
    for (i = 0; i < d15.length; i++) {
        if (d15.indexOf(num) == -1) {
            d15.push(num);
        }
    }
    showArr('.out-15', d15);
