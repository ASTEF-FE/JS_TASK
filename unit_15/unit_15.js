...
const f7 = () => {
    ...
    let arrPas7 = pas.split('');
    console.log(arrPas7);

    let setPas7 = new Set(arrPas7);
    console.log(setPas7);
    if (setPas7.size > 6 && setPas7) {
        console.log(1);
    } else {
        console.log(0);
    }
...
const f9 = our_set => {
    console.log([...our_set]); // set in array (es6). or let our_set1 = Array.from(our_set);
    return [...our_set].join(' '); // return our_set1.join(' ');
...
const f13 = () => {
    ...
    for (let i = 0; i < arr13.length; i++) {
        ...
        if (obj[a13]) { // Проверяем есть ли такой ключ в объекте
            obj[a13]++; // увеличивваем значение ключа на 1 (obj[a13] = obj[a13] + 1)
        } else {
            obj[a13] = 1; // ели такого ключа нет то добавлеем его и присваиваем значение 1
        }
