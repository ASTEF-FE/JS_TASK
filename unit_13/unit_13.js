...
function f18() {
    let out = '';
    let i181 = i18.value.toLowerCase();
    for (key in a18) {

        for (i = 0; i < a18[key].length; i++) {
            if (key.indexOf(i181) == 0) {
                out += ` ${a18[i181][i]} `;
                console.log(out);
            } else {
                out += '';
...
function f19() {
    ...
    for (let key in a19) {
        for (let i = 0; i < a19[key].length; i++) {
            if (a19[key][i] == i191 || a19[key][i].toLowerCase() == i191) {
                document.querySelector('.out-19').style.background = `${key}`;
