...
let ar2 = ['Artur', 'Stsefanovich', 20, true];
...
let ar3 = ['Artur', 'Stsefanovich', 20, true];
function f3() {
...
let ar6 = ['Артур', 'Дева', 14, 'сентябрь'];
...
let ar9 = [100, 200, 300, 400, 121, 700];
...
function f10() {
    ...
    for (i = 0; i < ar10.length; i++) {
        out10 = `${ar10[0] + ar10[ar10.length - 1]}`
    }
...
function f16() {
    ...
    let out16odd = '';
    let out16even = ''; //четные

    for (i = 0; i < ar16.length; i++) {
        if (ar16[i] % 2 == 0) {
            ar16_even[i] = ` ${ar16[i]} `;
            out16even += ar16_even[i];
        } else {
            ar16_odd[i] = ` ${ar16[i]} `;
            out16odd += ar16_odd[i];
        }
...
function f17() {
    ...
    document.querySelector('.out-17').innerHTML = key1;
}
